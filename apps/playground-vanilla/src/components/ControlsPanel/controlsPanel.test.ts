...
import { createControlsPanel } from '.'
...
describe('the controls panel against the public API', () => {
  ...
  it.each(HOOK_OPTION_KEYS)('marks %s as an option of the engine', (option) => {
    const section = getSection(renderPanel(), option)

    expect(section.className).toContain('pg-section--hook')
    expect(section.textContent).toContain('engine option')
  })
  ...
  it('puts every engine option in the section it is registered under', () => {
...
describe('the panel after a state change', () => {
  it('rereads the controls the new state disables', () => {
    const panel = createControlsPanel({
      state: { ...DEFAULT_STATE, slideWidth: 0 },
      update: vi.fn()
    })

    panel.update({
      state: { ...DEFAULT_STATE, slideWidth: 240 },
      update: vi.fn()
    })

    expect(getInput(panel.element, 'slidesNumber').disabled).toBe(true)
  })
})
