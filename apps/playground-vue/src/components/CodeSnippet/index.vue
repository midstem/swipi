  ...
  <section class="pg-card">
    ...
    <p class="pg-hint">
      <template v-if="minimal">
        The same carousel with everything optional taken off: no roles, no labels, no live region, no arrow keys — the layout as {{ tailwind ? 'Tailwind classes' : 'CSS' }}. Shortest thing that works; reach for the accessible variant before you ship.
      </template>
      <template v-else>
        Everything the current settings need: the hook options, the accessible markup around them, the rest as {{ tailwind ? 'Tailwind classes' : 'CSS' }}. The roles, labels and the live region are yours to edit and translate once you paste this.
      </template>
    </p>
...
const markup = computed(() => buildMarkup(props.state, minimal.value, tailwind.value))

const styles = computed(() => (tailwind.value ? '' : buildStyles(props.state, minimal.value)))

const copy = (): void => {
  const source = styles.value ? `${markup.value}\n\n/* CSS */\n${styles.value}` : markup.value

  void navigator.clipboard.writeText(source)
  copied.value = true
  setTimeout(() => { copied.value = false }, COPIED_TIMEOUT)
}
</script>
