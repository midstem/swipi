...
import { createFieldId, toRange, toStyle } from './helpers'
...
describe('createFieldId', () => {
  it('keeps the prefix the label points at', () => {
    expect(createFieldId('number-field')).toMatch(/^number-field-/)
  })

  it('hands every field an id of its own', () => {
    expect(createFieldId('text-field')).not.toBe(createFieldId('text-field'))
  })
})
