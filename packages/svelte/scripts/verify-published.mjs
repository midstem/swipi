...
const workdir = mkdtempSync(join(tmpdir(), 'swipi-svelte-consumer-'))
...
const verifyDevServer = async () => {
  ...
  try {
    ...
    if (!entry.includes('mount')) {
      throw new Error('the dev server does not transform the app entry')
    }

    const component = await readDevPage('/src/Carousel.svelte')

    if (!component.includes('deps/@midstem_swipi-svelte.js')) {
      throw new Error('vite did not pre-bundle swipi-svelte for the dev server')
    }

    if (!component.includes('svelte_internal_client')) {
      throw new Error(
    ...
    console.log(
      'dev: the dev server pre-bundles swipi-svelte and serves the app'
    )

    const hmr = await openHmrSocket()
    const componentPath = join(appDir, 'src', 'Carousel.svelte')
      ...
      (payload) =>
        ...
        payload.updates.some(
          (update) =>
            ...
            update.path === '/src/Carousel.svelte'
        ),
      HMR_TIMEOUT
    )
...
const main = async () => {
  ...
  step(`install the tarball on svelte ${SVELTE_VERSION}`)
  run(
    'npm',
    [
      'install',
  ...
  step(`entries, ssr and client on svelte ${SVELTE_VERSION}`)
