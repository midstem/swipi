...
check(
  `the ESM bundle inlines "${PEER_ENTRY}" instead of importing it`,
  new RegExp(`from\\s*['"]${PEER_ENTRY}['"]`).test(esm)
)

check(
  `the CJS bundle inlines "${PEER_ENTRY}" instead of requiring it`,
  new RegExp(`require\\(['"]${PEER_ENTRY}['"]\\)`).test(cjs)
)
